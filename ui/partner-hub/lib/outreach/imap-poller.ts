 ...
 * Power Automate relay notifications (shared by every campaign, since
 * there's one relay mailbox for all of them) and turns matched ones into
 * status updates + a TrackingEvent.
 *
 * Not run by any in-process timer -- app/api/outreach/imap-poll/route.ts
 * exposes this as a CRON_SECRET-protected endpoint, called on a schedule
 * by .github/workflows/outreach-imap-poll.yml. Nothing polls until
...
export async function pollImapForReplies(): Promise<PollResult> {
  ...
  const log = (msg: string) => console.log(`[outreach-imap-poll] +${Date.now() - pollStartedAt}ms ${msg}`);
