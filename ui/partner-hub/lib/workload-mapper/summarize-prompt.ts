...
export function buildSummarizePrompt(input: WorkloadSummaryRequest): string {
  ...
  return [
    ...
    "Then explain what information is missing and why it matters with concrete architecture/sizing impact.",
    ...
    "If latency is missing, explain that it affects scoring/retrieval response targets, serving design, caching, data placement, and real-time vs near-real-time architecture.",
    "If query concurrency is missing, explain analyst/job concurrency impact on compute sizing, throughput, cache design, and peak user experience.",
    "If exact data volume is missing, explain impact on storage sizing, tiering, data protection, and high-performance active data placement.",
    "If daily ingest rate is missing, explain impact on ingestion pipeline throughput, indexing, embedding, and time-to-searchability.",
    "If GPU requirements are missing, explain impact on accelerator pool sizing across training, inference, embeddings, and analytics.",
    "If HA/DR requirements are missing, explain impact on replication, failover, and recovery objectives.",
