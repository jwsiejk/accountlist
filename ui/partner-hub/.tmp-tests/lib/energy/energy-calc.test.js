...
const energy_calc_1 = require("./energy-calc");
const makePureRows = () => [
    {
...
(0, node_test_1.describe)("fbPower", () => {
    (0, node_test_1.it)("computes rack units and preserves energy outputs", () => {
        const result = (0, energy_calc_1.fbPower)(makePureRows(), 48, 5, 0.5, 1.2, 0.1, 2);
    ...
    (0, node_test_1.it)("returns null rack units when any component is missing RU", () => {
        const rows = makePureRows();
        rows[2] = { ...rows[2], Rack_Units: null };
        const result = (0, energy_calc_1.fbPower)(rows, 48, 5, 0.5, 1.2, 0.1, 2);
        assert.equal(result.rackUnits, null);
        assert.ok(Math.abs(result.weightedW - 300) < 1e-6);
    });
});
(0, node_test_1.describe)("NetApp candidates", () => {
    (0, node_test_1.it)("computes rack units in buildNetAppCandidate", () => {
        const candidate = (0, energy_calc_1.buildNetAppCandidate)(makeNetAppRows(), "C1", "DE460C 60-bay", 2, 0.5, 1.2, 0.1, 0.2, 0, 1.3, 18);
    ...
    (0, node_test_1.it)("returns null rack units when any NetApp RU input is missing", () => {
        const rows = makeNetAppRows();
        rows[0] = { ...rows[0], Rack_Units: null };
        const candidate = (0, energy_calc_1.buildNetAppCandidate)(rows, "C1", "DE460C 60-bay", 2, 0.5, 1.2, 0.1, 0.2, 0, 1.3, 18);
        assert.equal(candidate.rackUnits, null);
        assert.ok(Math.abs(candidate.weightedW - 375) < 1e-6);
    });
    (0, node_test_1.it)("computes rack units in enumerateNetApp", () => {
        const targetEffTb = 2695.68;
        const candidates = (0, energy_calc_1.enumerateNetApp)(makeNetAppRows(), targetEffTb, 0.5, 1.2, 0.1, 0.2, 0, 1.3, 18, 0.5);
...
(0, node_test_1.describe)("Vast candidates", () => {
    (0, node_test_1.it)("returns empty when Vast rows are missing", () => {
        const candidates = (0, energy_calc_1.enumerateVast)([], 100, 0.5, 1.2, 0.1, 0.1, 0, 1, 10, 0.05);
        assert.deepEqual(candidates, []);
    });
    (0, node_test_1.it)("returns the expected candidate within tolerance", () => {
        const targetEffTb = 432;
        const tolFrac = 0.02;
        const candidates = (0, energy_calc_1.enumerateVast)(makeVastRows(), targetEffTb, 0.5, 1.2, 0.1, 0.1, 0, 1, 10, tolFrac);
