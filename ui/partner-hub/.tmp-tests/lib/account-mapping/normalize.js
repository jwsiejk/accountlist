...
exports.blockingKeys = exports.blockingKey = exports.normalizeName = void 0;
...
const combineSingleLetterTokens = (tokens) => {
    const combined = [];
    let buffer = "";
...
const normalizeName = (raw) => {
    ...
    const stripped = tokens.filter((token) => !LEGAL_SUFFIXES.has(token) && !NOISE_TOKENS.has(token));
    return stripped.join(" ");
};
exports.normalizeName = normalizeName;
const blockingKey = (normalizedName) => {
    if (!normalizedName) {
        return "";
    }
...
exports.blockingKey = blockingKey;
const blockingKeys = (normalizedName) => {
    if (!normalizedName) {
        return [];
    }
    const tokens = normalizedName.split(/\s+/g).filter(Boolean);
...
exports.blockingKeys = blockingKeys;
