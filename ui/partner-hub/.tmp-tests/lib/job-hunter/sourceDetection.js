...
exports.detectSourceFromUrl = void 0;
const toSlug = (value) => value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
const prettyCompany = (value) => {
...
const detectSmartRecruiters = (url) => {
    const host = normalizeHost(url.hostname);
    const parts = url.pathname.split("/").filter(Boolean);
